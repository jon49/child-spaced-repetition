...
Your Controller will have access to all its views through a variable in the Controller called `$this->view`. This variable is provided by `AppController` and holds the entire hierarchy of Sub Views. You can send variables from the controller to the View or any of it's Sub Views as follows
...
The View objects are smart and know when you're trying to access a View or Sub View that exists vs when you're trying to make a variable. In the case of `$this->view->foo`, it knows that there isn't a Sub Template called `foo` and therefore you're making a new variable. In the case of `$this->view->primary_header->bar `, it knows that `primary_header` is a Sub View of the main View and therefore allows you to pass new variables into that Sub View (like `bar`). Note also that passing variables to `$this->view` will make those variables available in the `master.php` template since that's how our Default View is organized.

Besides passing variables into specific Views, you'll also want to pass "page specific content" into the Views and into `master.php`. Take note that all output created in your controller will end up

Further, all output generated by the controller will turn into a variable called
